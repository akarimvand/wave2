    ...
    <div class="app-layout">
        ...
        <aside id="sidebar" class="sidebar">
            <div class="sidebar-brand">
                <?php $clubLogoP = getSetting('logo_path'); if ($clubLogoP): ?>
                <img src="<?php echo asset($clubLogoP); ?>" alt="<?php echo e(getSetting('club_name', 'WAVE')); ?>" class="sidebar-brand-logo">
                <?php else: ?>
            ...
            <nav class="sidebar-nav">
                <a href="<?php echo url('portal/dashboard'); ?>" class="<?php echo (isset($activeMenu) && $activeMenu === 'dashboard') ? 'active' : ''; ?>">
                    <i class="fas fa-home"></i>
                    داشبورد
                </a>

                <!-- حساب کاربری -->
                <div class="sidebar-accordion <?php echo ($activeMenu ?? '') === 'profile' ? 'open' : ''; ?>">
                    <button type="button" class="sidebar-accordion-header" onclick="this.parentElement.classList.toggle('open')">
                        <i class="fas fa-user acc-icon"></i>
                        حساب کاربری
                        <i class="fas fa-chevron-down acc-arrow"></i>
                    </button>
                    <div class="sidebar-accordion-body">
                        <a href="<?php echo url('portal/profile'); ?>" class="<?php echo ($activeMenu ?? '') === 'profile' ? 'active' : ''; ?>">
                            پروفایل
                        </a>
                    </div>
                </div>
                ...
                <!-- باشگاه -->
                <div class="sidebar-accordion <?php echo in_array($activeMenu ?? '', ['classes', 'attendance', 'insurance']) ? 'open' : ''; ?>">
                    <button type="button" class="sidebar-accordion-header" onclick="this.parentElement.classList.toggle('open')">
                        <i class="fas fa-dumbbell acc-icon"></i>
                        باشگاه
                        <i class="fas fa-chevron-down acc-arrow"></i>
                    </button>
                    <div class="sidebar-accordion-body">
                        <a href="<?php echo url('portal/classes'); ?>" class="<?php echo ($activeMenu ?? '') === 'classes' ? 'active' : ''; ?>">
                            کلاس‌ها
                        </a>
                        <a href="<?php echo url('portal/attendance'); ?>" class="<?php echo ($activeMenu ?? '') === 'attendance' ? 'active' : ''; ?>">
                            حضور و غیاب
                        </a>
                        <a href="<?php echo url('portal/insurance'); ?>" class="<?php echo ($activeMenu ?? '') === 'insurance' ? 'active' : ''; ?>">
                            بیمه
                        </a>
                    </div>
                </div>
                ...
                <!-- مالی و ارتباطات -->
                <div class="sidebar-accordion <?php echo in_array($activeMenu ?? '', ['payments', 'tickets', 'notifications']) ? 'open' : ''; ?>">
                    <button type="button" class="sidebar-accordion-header" onclick="this.parentElement.classList.toggle('open')">
                        <i class="fas fa-wallet acc-icon"></i>
                        مالی و ارتباطات
                        <i class="fas fa-chevron-down acc-arrow"></i>
                    </button>
                    <div class="sidebar-accordion-body">
                        <a href="<?php echo url('portal/payments'); ?>" class="<?php echo ($activeMenu ?? '') === 'payments' ? 'active' : ''; ?>">
                            پرداخت‌ها
                        </a>
                        <a href="<?php echo url('portal/tickets'); ?>" class="<?php echo ($activeMenu ?? '') === 'tickets' ? 'active' : ''; ?>">
                            تیکت‌ها
                        </a>
                        <a href="<?php echo url('portal/notifications'); ?>" class="<?php echo ($activeMenu ?? '') === 'notifications' ? 'active' : ''; ?>">
                            اعلانات
                        </a>
                    </div>
                </div>
            ...
            <header class="topbar">
                <div class="topbar-right">
                    ...
                    <span class="topbar-user" style="display:flex;align-items:center;gap:8px;">
                        <?php $avatarP = auth()->user()['avatar_path'] ?? ''; $uNameP = auth()->user()['full_name'] ?? auth()->user()['username'] ?? 'عضو'; ?>
                        <?php if (!empty($avatarP)): ?>
                        <img src="<?php echo asset($avatarP); ?>" alt="" style="width:32px;height:32px;border-radius:50%;object-fit:cover;border:2px solid var(--app-primary);">
                        <?php else: ?>
                        <span style="width:32px;height:32px;border-radius:50%;background:var(--app-gradient);display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:0.8rem;font-weight:700;flex-shrink:0;"><?php echo e(mb_substr($uNameP, 0, 1)); ?></span>
                        <?php endif; ?>
                        <?php echo e($uNameP); ?>
                    </span>
                </div>
